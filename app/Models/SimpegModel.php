...
class SimpegModel extends Model
{
  ...
  {
    if($kode == 0){
      $query = $this->db->query("SELECT JENIS_KELAMIN, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE JENIS_KELAMIN IS NOT NULL AND STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') GROUP BY JENIS_KELAMIN");
    }else{
      $query = $this->db->query("SELECT JENIS_KELAMIN, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') AND KODE_SATUAN_KERJA LIKE '$kode%' GROUP BY JENIS_KELAMIN");
    }
  ...
  {
    $query = $this->db->query("SELECT TIPE_JABATAN, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE TIPE_JABATAN IS NOT NULL AND STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') GROUP BY TIPE_JABATAN ORDER BY TIPE_JABATAN");
    // if($kode == 0){
    // }else{
    //   $query = $this->db->query("SELECT LEVEL_JABATAN, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE LEVEL_JABATAN IS NOT NULL AND STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') AND KODE_SATUAN_KERJA LIKE '$kode%' GROUP BY LEVEL_JABATAN ORDER BY LEVEL_JABATAN");
    // }
  ...
  {
    if($kode == 0){
      $query = $this->db->query("SELECT AGAMA, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE AGAMA IS NOT NULL AND STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') GROUP BY AGAMA ORDER BY AGAMA");
    }else{
      $query = $this->db->query("SELECT AGAMA, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE AGAMA IS NOT NULL AND STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') AND KODE_SATUAN_KERJA LIKE '$kode%' GROUP BY AGAMA ORDER BY AGAMA");
    }

    return $query->getResult();
  }

  public function dashboard_status($kode)
  {
    if($kode == 0){
      $query = $this->db->query("SELECT STATUS_PEGAWAI, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') GROUP BY STATUS_PEGAWAI ORDER BY STATUS_PEGAWAI");
    }else{
      $query = $this->db->query("SELECT STATUS_PEGAWAI, COUNT(NIP) AS JUMLAH FROM TEMP_PEGAWAI WHERE STATUS_PEGAWAI IN ('CPNS','PNS','PPPK') AND KODE_SATUAN_KERJA LIKE '$kode%' GROUP BY STATUS_PEGAWAI ORDER BY STATUS_PEGAWAI");
    }
  ...
  {
    if($kode == 0){
      $query = $this->db->query("SELECT LEVEL_JABATAN AS JABATAN, COUNT(NIP_BARU) AS JML FROM TEMP_PEGAWAI WHERE LEVEL_JABATAN IS NOT NULL GROUP BY LEVEL_JABATAN");
    }else{
      $query = $this->db->query("SELECT LEVEL_JABATAN AS JABATAN, COUNT(NIP_BARU) AS JML FROM TEMP_PEGAWAI WHERE LEVEL_JABATAN IS NOT NULL AND KODE_SATKER_2='$kode' GROUP BY LEVEL_JABATAN");
    }
